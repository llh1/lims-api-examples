    ...
    module ManualExtraction
      ...
      def manual_extraction
        ...
        2.times do
          ...
          new_assets << {:read => response["spin_column"]["actions"]["read"], :type => "spin_column", :content => "DNA", :uuid => response["spin_column"]["uuid"]}

          API::new_step("Create a new tube")
          parameters = {:tube => {}}
          response = API::post(@routes["tubes"]["actions"]["create"], parameters)
          new_assets << {:read => response["tube"]["actions"]["read"], :type => "tube", :content => "RNA+P", :uuid => response["tube"]["uuid"]}
        end

        # Create 4 barcodes
        barcode_values = []
        new_assets.each do |e|
        ...
        new_assets_uuids = [[new_assets[0][:uuid], new_assets[1][:uuid]], [new_assets[2][:uuid], new_assets[3][:uuid]]]
        ...
        transfer_uuids = [
          [initial_tube_uuids[0], new_assets_uuids[0][0], new_assets_uuids[0][1]], 
          [initial_tube_uuids[1], new_assets_uuids[1][0], new_assets_uuids[1][1]]
        ] 
